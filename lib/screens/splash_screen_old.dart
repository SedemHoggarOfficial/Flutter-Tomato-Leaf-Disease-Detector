...
class _SplashScreenState extends State<SplashScreen>
  ...
  void initState() {
    ...
    // Start splash animation and model loading in parallel
    _startSequence();
    _initializeModels();
  }
  ...
  /// Initialize ML models in background isolates
  /// This happens in parallel with the splash animation
  Future<void> _initializeModels() async {
    try {
      print('🚀 [Splash] Starting model initialization in background...');
      final success = await GlobalModelManager().initialize();

      if (mounted) {
        if (success) {
          print('✅ [Splash] Models initialized successfully');
          setState(() => _modelsLoaded = true);

          // If animations are done, navigate immediately
          if (!_scanSequenceController.isAnimating) {
            _navigateAway();
          }
        } else {
          final error = GlobalModelManager().initializationError;
          throw error ?? Exception('Failed to initialize models');
        }
      }
    } catch (e) {
      print('❌ [Splash] Model initialization error: $e');
      if (mounted) {
        setState(() {
    ...
    _mainController.forward().then((_) {
      ...
      _scanSequenceController.forward().then((_) {
        ...
        Future.delayed(const Duration(milliseconds: 2000), () {
          // Wait for models to load before navigating
          if (_modelsLoaded) {
            _navigateAway();
          } else if (_modelLoadingError) {
            _showErrorDialog();
          } else {
            // Models still loading, wait a bit more
    ...
    if (mounted) {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text(
            'Failed to load AI models: $_modelLoadingErrorMessage\n\nPlease try restarting the app.',
          ),
          actions: [
            TextButton(
              onPressed: () {
  ...
  void _navigateAway() {
    if (mounted && _modelsLoaded && !_modelLoadingError) {
      _scanSequenceController.stop();
      _flowController.stop();
      _pulseController.stop();
      _vibrationController.stop();
      ...
      child: FadeTransition(
        ...
        child: Column(
          children: [
            ...
            if (!_modelsLoaded && !_modelLoadingError)
              Column(
                children: [
                  SizedBox(
                    width: 200,
